...
export const generatePhoneToken = async (
	email: string,
	type: string
): Promise<string> => {
	return type === 'VERIFY_EMAIL'
		? `${email.toUpperCase().charCodeAt(0)}${email.toUpperCase().charCodeAt(1)}`
		: `${email.toUpperCase().charCodeAt(1)}${email.toUpperCase().charCodeAt(0)}`
}
